...
const AdminVehicles = () => {
    ...
    useEffect(() => {
        fetchVehicles();
    }, [page, vehicleTypeFilter, statusFilter, searchQuery]);
    ...
    const getVehicleTypeBadgeVariant = (type) => type === 'Two Wheeler' ? 'info' : 'warning';
    ...
    return (
        <div className="space-y-6 p-4 md:p-8">
            ...
            {/* Premium Header */}
            <div className="rounded-2xl bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-8 shadow-xl text-white">
                <h1 className="text-3xl font-extrabold mb-2 text-center">Vehicle Management</h1>
                <p className="text-center text-lg text-white/90 mb-6">
                    View and manage all registered vehicles in the society
                </p>
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-4">
                    ...
                    <div className="flex flex-col">
                        <label className="text-sm font-medium mb-1">Filter by Vehicle Type</label>
                        <select
                    ...
                    <div className="flex flex-col flex-grow">
                        <label className="text-sm font-medium mb-1">Search Vehicles</label>
            ...
            <Card>
                <CardContent>
                            ...
                            <table className="w-full table-auto border-collapse border border-gray-200">
                                ...
                                {vehicles.map((vehicle) => (
                                    <tr key={vehicle._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="border border-gray-300 px-4 py-2">{vehicle.userId?.fullName || 'Unknown'}</td>
                                        <td className="border border-gray-300 px-4 py-2">{vehicle.userId ? `${vehicle.userId.wing}-${vehicle.userId.flatNumber}` : 'N/A'}</td>
                                        <td className="border border-gray-300 px-4 py-2">
                                        ...
                                        <td className="border border-gray-300 px-4 py-2">
                                            <div className="flex flex-wrap gap-2">
                                                <Button onClick={() => openVehicleModal(vehicle)} variant="default" size="sm">
                                                    View Details
                                                </Button>
                                                {vehicle.status === 'pending' && (
                                                    <>
                                                        <Button onClick={() => openStatusDialog(vehicle, 'approve')} variant="success" size="sm">
                                                            Approve
                                                        </Button>
                                                        <Button onClick={() => openStatusDialog(vehicle, 'reject')} variant="destructive" size="sm">
                                                            Reject
                                                        </Button>
                                                    </>
                                                )}
                    ...
                    <div className="mt-4 flex justify-center space-x-2">
                        <Button onClick={() => setPage((p) => Math.max(p - 1, 1))} disabled={page === 1} size="sm">
                            Previous
                        </Button>
                        <span className="px-3 py-2 border border-gray-300 rounded">{page} / {totalPages}</span>
                        <Button onClick={() => setPage((p) => Math.min(p + 1, totalPages))} disabled={page === totalPages} size="sm">
                            Next
                        </Button>
                    </div>
                </CardContent>
                ...
                <ModalBody>
                    {selectedVehicle && (
                        <div className="space-y-4">
                            ...
                            <p><strong>Registration Date:</strong> {formatDate(selectedVehicle.registrationDate)}</p>
                            ...
                            <p><strong>Flat Number:</strong> {selectedVehicle.userId ? `${selectedVehicle.userId.wing}-${selectedVehicle.userId.flatNumber}` : 'N/A'}</p>
            ...
            <AlertDialog open={statusDialogOpen} onOpenChange={setStatusDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>{statusAction === 'approve' ? 'Approve Vehicle' : 'Reject Vehicle'}</AlertDialogTitle>
                    ...
                    <AlertDialogFooter>
                        ...
                        >
                            {processingStatus ? 'Processing...' : statusAction === 'approve' ? 'Approve' : 'Reject'}
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
